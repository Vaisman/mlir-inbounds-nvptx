...
module attributes {gpu.container_module} {
  gpu.module @kernels [#nvvm.target<chip = "sm_80">] {
    gpu.func @tensorcore_in_bounds(
        %a: memref<?x?xf16, #gpu.address_space<workgroup>>,
        %b: memref<?x?xf16, #gpu.address_space<workgroup>>,
        %c: memref<?x?xf16, #gpu.address_space<workgroup>>) kernel {
      %c0 = arith.constant 0 : index
      %f0 = arith.constant 0.0 : f16
      %A = vector.transfer_read %a[%c0, %c0], %f0
          {in_bounds = [true, true]}
          : memref<?x?xf16, #gpu.address_space<workgroup>>,
            vector<16x16xf16>
      %B = vector.transfer_read %b[%c0, %c0], %f0
          {permutation_map = #rowcol, in_bounds = [true, true]}
          : memref<?x?xf16, #gpu.address_space<workgroup>>,
            vector<8x16xf16>
      ...
      %D = vector.contract {
          ...
          : vector<16x16xf16>, vector<8x16xf16>
            into vector<16x8xf16>
